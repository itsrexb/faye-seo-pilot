...
final class OpenAiClient implements AiClientInterface {
	...
	public function send( string $system_prompt, string $user_content, int $max_tokens = 4096 ): array|\WP_Error {
		if ( empty( $this->api_key ) ) {
			return new \WP_Error(
				'seopilot_no_api_key',
				__( 'OpenAI API key is not configured. Please set it in SEO Pilot Pro → Settings.', 'seo-pilot-pro' )
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'API request failed: %s', 'seo-pilot-pro' ),
		...
		if ( $status === 401 ) {
			return new \WP_Error( 'seopilot_auth_error', __( 'Invalid OpenAI API key.', 'seo-pilot-pro' ) );
		}

		if ( $status === 429 ) {
			return new \WP_Error( 'seopilot_rate_limit', __( 'OpenAI API rate limit reached. Please wait and try again.', 'seo-pilot-pro' ) );
		}

		if ( $status !== 200 ) {
			/* translators: %d: HTTP status code */
			$error_msg = $data['error']['message'] ?? sprintf( __( 'API returned HTTP %d.', 'seo-pilot-pro' ), $status );
			return new \WP_Error( 'seopilot_api_error', $error_msg );
		}

		$text = $data['choices'][0]['message']['content'] ?? '';
		if ( empty( $text ) ) {
			return new \WP_Error( 'seopilot_empty_response', __( 'OpenAI returned an empty response.', 'seo-pilot-pro' ) );
		}
